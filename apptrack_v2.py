...
class ApptrackDB:
    ...
    def add(self, role, company, date_applied, job_link, status, notes):
        if company == None:
    ...
    def update(self, id, status, date_applied, job_link, notes):
        conn, cur = self.get_conn()
        did_update = False
        cur.execute('''
        ...
        if not exists:
            ...
            if not date_applied is None:
                cur.execute('''
                UPDATE applications SET date_applied = ? WHERE id = ?
                ''', (date_applied, id))
                did_update = True

            if not job_link is None:
                job_link = clean_job_link(job_link)
                cur.execute('''
...
if __name__ == "__main__":
    ...
    if args.cmd == "add":
        db.add(args.role, args.company, args.date_applied, args.job_link, args.status, args.notes)
    ...
    # update branch 
        ...
        db.update(args.id, args.status, args.date_applied, args.job_link, args.notes)
